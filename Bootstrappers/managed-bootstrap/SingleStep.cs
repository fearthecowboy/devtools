...
namespace CoApp.Bootstrapper {
    ...
    internal class SingleStep {
        public static string MIN_COAPP_VERSION_STRING = "1.2.0.358";
        public static bool IsForcingCoappToClean = false;
        public static ulong INCOMPATIBLE_VERSION = AssemblyCacheEnum.VersionStringToUInt64("1.2.0.357");

        public static string[] AssemblyNames = {
            "CoApp.Toolkit", "CoApp.Client", "CoApp.Toolit.Engine.Client"
        };
        ...
        internal static bool PassiveOrQuiet {
            get {
        ...
        static SingleStep() {
            Progress = new MultifactorProgressTracker {
                ResourceDllDownload,
                CoAppPackageDownload,
                CoAppPackageInstall,
                EngineStartup
            };
        ...
        public static void Main(string[] args) {
            ...
            if (args.Length > 1) {
                foreach (var split in args.Skip(1).Select(a => a.ToLower().Split(new[] {
                    '='
                }, StringSplitOptions.RemoveEmptyEntries)).Where(split => split.Length >= 2)) {
                    if (split[0] == "--uilevel") {
        ...
        internal static void RunInstaller(bool bypassingBootstrapUI) {
            ...
            try {
                ...
                MainWindow.WhenReady += () => {InstallerStageTwo(appDomain);};
        ...
        private static void InstallerStageTwo(AppDomain appDomain) {
            try {
                ...
                Task.Factory.StartNew(() => {
                    ping.WaitOne();
                    MainWindow.WhenReady += () => {MainWindow.MainWin.Visibility = Visibility.Hidden;};
                });
                ...
                if (File.Exists(@"c:\root\coapp\coapp\output\any\debug\bin\CoApp.Client.dll")) {
                    ...
                    appDomain.CreateInstanceFromAndUnwrap(@"c:\root\coapp\coapp\output\any\debug\bin\CoApp.Client.dll", "CoApp.Packaging.Client.UI.Installer", false, BindingFlags.Default, null, new[] {
                        MsiFilename
                    }, null, null);
                ...
                appDomain.CreateInstanceAndUnwrap("CoApp.Client, Version=" + MIN_COAPP_VERSION_STRING + ", Culture=neutral, PublicKeyToken=1e373a58e25250cb",
                    "CoApp.Packaging.Client.UI.Installer", false, BindingFlags.Default, null, new[] {
                        MsiFilename
                    }, null, null);
                ...
                try {
                    ...
                    if (!Cancelling) {
                        ...
                        if (IsIncompatibleCoAppInstalled) {
                            if (!PassiveOrQuiet) {
                                // skip the UI if we're in quiet or passive mode.
